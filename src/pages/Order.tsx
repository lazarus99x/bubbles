...
const Order: React.FC = () => {
  ...
  const getTotalPrice = () => {
    return selectedItems.reduce((total, item) => total + (item.dish.price * item.quantity), 0);
  };
      ...
      <div className="section-padding">
        <div className="container mx-auto px-4 max-w-6xl">
          ...
          <div className="text-center mb-8">
            <h1 className="text-4xl font-bold text-bubbles-pink mb-2">Place Your Order</h1>
            <p className="text-gray-600">Select your favorite items and order via WhatsApp</p>
          </div>
                ...
                <div key={category} className="mb-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {categoryDishes.map((dish) => {
                      ...
                      return (
                        <div key={dish.id} className="bg-white rounded-lg shadow-md p-4 border">
                          {dish.image_url && (
                            <img
                              src={dish.image_url}
                              alt={dish.name}
                ...
                <div className="bg-white rounded-lg shadow-lg p-6 border">
                  ...
                  {selectedItems.length === 0 ? (
                    ...
                    <div className="space-y-3 mb-4">
                      {selectedItems.map((item) => (
                        <div key={item.dish.id} className="flex justify-between items-center py-2 border-b">
                          <div className="flex-1">
                            <p className="font-medium">{item.dish.name}</p>
                            <p className="text-sm text-gray-600">
                              {item.quantity}x ₦{item.dish.price.toLocaleString()}
                            </p>
                          </div>
                      ...
                      <div className="flex justify-between items-center pt-2 text-lg font-bold">
                        <span>Total:</span>
                        <span className="text-bubbles-pink">₦{getTotalPrice().toLocaleString()}</span>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg mb-4">
                    <p className="text-sm text-blue-800">
                      <strong>Delivery:</strong> Fees calculated based on location. 
                      We'll confirm availability and total cost via WhatsApp.
                    </p>
                  </div>
                  ...
                  <Button
                    ...
                    disabled={selectedItems.length === 0 || !customerName.trim() || !customerAddress.trim()}
